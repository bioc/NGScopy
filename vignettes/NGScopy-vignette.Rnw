...
%\VignetteIndexEntry{NGScopy: Detection of copy number variations in next generation sequencing: case studies and functionality}
...
\newcommand{\nsegtype}{%
<<echo=FALSE,eval=TRUE,results=tex>>=%
  cat(length(NGScopy::parse_segtype()))
@%
}
...
\vspace{6ex}
In the above code, we use the data provided in \Bioconductor{} \NGScopyData{} package \citeNGScopyData{}. We first create an instance of \code{NGScopy} class, an \R{} reference class \cite{Chambers2014un-R}. Calling the method \code{write_cn} automatically calls the method \code{proc_normal}, \code{proc_tumor}, \code{calc_cn}, \code{proc_cn} and \code{write_cn} in a row to perform window making, read counting in both samples, CNR computing and results processing and saves it in a tab separated file. Calling the method \code{write_seg} automatically calls the method \code{calc_seg}, \code{proc_seg} and \code{write_seg} in a row to perform segmentation and results processing and saves it in a tab separated file. Finnaly we visualize the results and save it in a pdf file.
...
obj$plot_cn(ylim=c(-3,3))       # reset `ylim' to NULL to allow full-scale display
@ 
\end{mdframed}
...
\begin{mdframed}
<<echo=TRUE,eval=TRUE,results=verbatim>>=
## NGScopy::help_segtype("mean.norm")
@
\end{mdframed}
...
obj$set_segtype("mean.norm,meanvar.norm,mean.cusum,var.css")
@  
...
\vignetteSource{sh/ngscopy-case1b-cmdline.sh}{ngscopy-case1b-cmdline.sh}

The output,
\vignetteSource{sh/ngscopy-case1b-cmdline.out}{ngscopy-case1b-cmdline.out}
